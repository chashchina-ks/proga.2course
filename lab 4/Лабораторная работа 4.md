...
### Пояснение к решению
- bin_2.py
1. Функция gen_bin_tree(height=5, root=10, left_leaf=None, right_leaf=None):

Принимает высоту дерева (`height`) и значение корня (`root`) и функции для вычисления левого/правого потомка (`left_leaf`/`right_leaf`) в качестве аргументов. Функция возвращает словарь, хранящий в себе бинарное дерево
...
4.  tree[node]['left'] = left_value и tree[node]['right'] = right_value:

Добавляют ссылки на левого и правого потомков к текущему узлу в словаре

5.  next_level_nodes.append(left_value) и next_level_nodes.append(right_value):
